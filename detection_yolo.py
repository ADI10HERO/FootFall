...
def dectection_video_file(webcam, video_path, yolo_weights, yolo_cfg, coco_names, confidence_threshold, nms_threshold):
    ...
    try:
        ...
        video.release()
        print("Average inference time per frame calculated over {} frames is: {}".format(len(ir_times), sum(ir_times)/len(ir_times)))
...
if __name__ == '__main__':
    ...
    ap.add_argument('--weights', help='Path to model weights', type=str, default='detector/yolo/yolov3.weights')
    ap.add_argument('--configs', help='Path to model configs',type=str, default='detector/yolo/yolov3.cfg')
    ap.add_argument('--class_names', help='Path to class-names text file', type=str, default='detector/yolo/coco.names')
